...
public class ChatDAOTest {
    ...
    public void setUp() {
        ...
        channel = new ChatChannel(
                ...
                "admin", null, null);
        Assert.assertTrue(dao.saveChannel(channel));
    }
    ...
    public void lastActivityIsTheMessageIdSoAReadCursorCanCatchUp() {
        final ChatMessage saved = dao.saveMessage(draft("hello"), channel, null).orElseThrow();

        final Map<String, String> activity = dao.lastActivity();
    ...
    public void saveAndPollRoundTrip() {
        ...
        final Optional<ChatMessage> saved = dao.saveMessage(draft, channel, null);
        ...
        final ChatPage empty = dao.getMessagesSince(
                channel.getId(), page.getCursor(), 50, null, channel, null, Instant.now());
        Assert.assertTrue(empty.getMessages().isEmpty());
    }

    @Test
    public void newestFirstPaging() {
        dao.saveMessage(draft("a"), channel, null);
        dao.saveMessage(draft("b"), channel, null);
        dao.saveMessage(draft("c"), channel, null);

        final ChatPage page = dao.getMessagesBefore(
                channel.getId(), null, 2, null, channel, null, Instant.now());
    ...
    public void tombstoneClearsBody() {
        final ChatMessage saved = dao.saveMessage(draft("secret"), channel, null).orElseThrow();
        final ChatMessage tomb = dao.tombstoneMessage(channel.getId(), saved.getId(), "admin")
                .orElseThrow();
    ...
    public void expiredItemInvisibleEvenIfStillInTable() {
        ...
        dao.saveChannel(shortChannel);
        final ChatMessage saved = dao.saveMessage(draft("gone soon"), shortChannel, null).orElseThrow();

        // Force "now" past expiry
        final Instant later = saved.getSentAt().plusSeconds(10);
        final ChatPage page = dao.getMessagesSince(
                shortChannel.getId(), null, 50, null, shortChannel, null, later);
        ...
        final Optional<ChatMessage> raw = dao.getMessage(shortChannel.getId(), saved.getId());
    ...
    public void collisionRetryAdvancesToTheNextFreeMillis() {
        ...
        for (int i = 0; i < band; i++) {
            occupied.add(dao.saveMessage(draft("occupier" + i), channel, null).orElseThrow());
        }
        ...
        dao.forceAllocator(base - 1);
        final ChatMessage collided = dao.saveMessage(draft("collided"), channel, null).orElseThrow();
        ...
        final ChatPage page = dao.getMessagesSince(
                channel.getId(), null, 100, null, channel, null, Instant.now());
    ...
    public void exhaustingTheRetryCapReportsFailureRatherThanDropping() {
        ...
        for (int i = 0; i < 30; i++) {
            dao.saveMessage(draft("occupier" + i), channel, null).orElseThrow();
        }
        dao.forceAllocator(base - 1);
        Assert.assertTrue(dao.saveMessage(draft("hopeless"), channel, null).isEmpty(),
    ...
    public void staleCursorReadsThroughToDynamoRatherThanSkippingTheGap() {
        final ChatMessage oldest = dao.saveMessage(draft("day one"), channel, null).orElseThrow();
        dao.saveMessage(draft("day two"), channel, null);
        final ChatMessage newest = dao.saveMessage(draft("day three"), channel, null).orElseThrow();
        ...
        final ChatPage page = dao.getMessagesSince(
                channel.getId(), oldest.getId(), 50, null, channel, null, Instant.now());
    ...
    public void missingBodyFallsBackToDynamoInsteadOfDroppingTheMessage() {
        final ChatMessage first = dao.saveMessage(draft("kept"), channel, null).orElseThrow();
        final ChatMessage second = dao.saveMessage(draft("body evicted"), channel, null).orElseThrow();

        cache.removeHashField(CacheKeys.chatBodyKey(channel.getId().getValue()),
                second.getId().getValue());

        final ChatPage page = dao.getMessagesSince(
                channel.getId(), first.getId(), 50, null, channel, null, Instant.now());
        Assert.assertEquals(page.getMessages().size(), 1);
    ...
    public void cursorAdvancesPastFilteredOutMessages() {
        ...
        final ChatChannel shortChannel = channel.withSettings(shortRetention);
        dao.saveChannel(shortChannel);
        final ChatMessage expiring = dao.saveMessage(draft("expires"), shortChannel, null).orElseThrow();
    ...
    public void getVisibleMessageHidesWhatTheFeedWouldHide() {
        ...
        final ChatChannel shortChannel = channel.withSettings(shortRetention);
        dao.saveChannel(shortChannel);
        final ChatMessage saved = dao.saveMessage(draft("gone"), shortChannel, null).orElseThrow();
        final Instant later = saved.getSentAt().plusSeconds(30);

        Assert.assertTrue(dao.getMessage(shortChannel.getId(), saved.getId()).isPresent(),
                "the unfiltered moderation read still sees it");
        Assert.assertTrue(dao.getVisibleMessage(
                        shortChannel.getId(), saved.getId(), null, shortChannel, null, later).isEmpty(),
    ...
    public void historyPageIsMarkedNewestFirstAndItsCursorPointsBackwards() {
        final ChatMessage oldest = dao.saveMessage(draft("a"), channel, null).orElseThrow();
        dao.saveMessage(draft("b"), channel, null);
        dao.saveMessage(draft("c"), channel, null);
    ...
    public void clientMessageIdIsIdempotent() {
        ...
        final ChatMessage a = dao.saveMessage(draft, channel, null).orElseThrow();
        final ChatMessage b = dao.saveMessage(draft, channel, null).orElseThrow();
        ...
        cache.subscribe(List.of(CacheKeys.chatPubSubChannelFor(channel.getId().getValue())),
                (ch, payload) -> {
                    ...
                    seen[0] = dao.getMessagesSince(channel.getId(), null, 50, null, channel, null, Instant.now())
                            .getMessages().stream().findFirst().orElse(null);
                });

        final ChatMessage saved = dao.saveMessage(draft("ring the bell"), channel, null).orElseThrow();
